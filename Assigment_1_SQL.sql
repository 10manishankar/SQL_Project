...
/*4. Retrieve the names of all the Bollywood movies which have been released over 
more than 1000 number of screens in the dataset.*/
...
/* 5. Retrieve the names of all the Bollywood movies which have generated
 ...
 /* 6. Retrieve the names of all the Bollywood movies which have budget
 less than 1cr in the dataset.*/
 ...
 select movie_name, revenue, budget, (revenue - budget) as flop from movies_sql where (revenue-budget)<0;
 /* 8. Retrieve the names and profit of all the Bollywood movies in the dataset.(profit=revenue – budget)*/
 select movie_name, revenue, budget, (revenue - budget) as profit from movies_sql where (revenue-budget)>0;
 /* 9. Retrieve the names and loss of all the Bollywood movies in the dataset.(loss=revenue – budget) */
  select movie_name, (revenue - budget) as loss from movies_sql where (revenue-budget)<0;
 ...
 /*11. Retrieve the names of all the Bollywood movies which have lead star as Akshay Kumar and
 directed by Priyadarshan in the dataset.*/
 ...
 select movie_name from movies_sql where movie_name like '%a';
 /*14. Retrieve the names of all the Bollywood movies having ‘a’ at second place of the name
 in the dataset.*/
 ...
 /* 20. Retrieve the names and budget of all the Bollywood movies according to the
 highest budget to lowest budget in the dataset.*/
 ...
 /* 21. Retrieve the names and budget of top 5 Bollywood movies with highest budget in the dataset.*/
  ...
  select movie_name, (revenue - budget) as flop from movies_sql where (revenue-budget)<0 order by (revenue-budget)
  asc limit 5;
  /* 25. Retrieve the names of top 5 hit movies in the dataset. */
  select movie_name, (revenue - budget) as hit from movies_sql where (revenue-budget)>0 order by (revenue-budget)
  desc limit 5;
  ...
  select movie_name, lead_star, budget from movies_sql where lead_star= 'Amitabh Bachchan' order by budget 
  desc limit 1,1;
  /* 29. Which are the flopped movies of Akshay Kumar. */
  select movie_name, lead_star, (revenue-budget) as flop from movies_sql where lead_star= 'Akshay Kumar' and 
  (revenue-budget) < 0;
  /* 30. With which director Sharukh Khan have given the biggest hit movie. */
  select movie_name, lead_star, director, (revenue-budget) as hit from movies_sql where lead_star= 'Shahrukh Khan'
